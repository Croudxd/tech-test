...
TEST(TestFxTradeLoadAccuracyOfFirstTrade) {
    ...
    valueTm.tm_mon = 10 - 1;
    valueTm.tm_mday = 11;
    auto expectedValueDate = std::chrono::system_clock::from_time_t(std::mktime(&valueTm));
    auto actualValueDate = trade->getValueDate();
    auto valueDiff = std::chrono::duration_cast<std::chrono::hours>(actualValueDate - expectedValueDate).count();
